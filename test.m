% Meyve tespiti için görüntü iþleme algoritmalarýyla projenin kodu

% Dosyadan meyve resimlerini yükleme
meyveDosyaAdi = 'C:\MATLAB Projects\meyve_resimler';
resimler = dir(fullfile(meyveDosyaAdi, '*.jpg'));
meyveSayisi = length(resimler);
meyveTurleri = cell(meyveSayisi, 1);
for i = 1:meyveSayisi
    [~, dosyaAdi, ~] = fileparts(resimler(i).name);
    meyveTurleri{i} = dosyaAdi;
end
% Buton ile meyve seçimi
figure;
selecButon = uicontrol('Style', 'pushbutton', 'String', 'DOSYADAN MEYVE SEÇ', ...
    'Position', [180 220 220 40], 'Callback', @meyveSec);
function meyveSec(~, ~)
    % Dosyadan meyve resimlerini yükleme
    meyveDosyaAdi = 'C:\MATLAB Projects\meyveler\meyve_resimler';
    resimler = dir(fullfile(meyveDosyaAdi, '*.jpg'));
    meyveSayisi = length(resimler);
    meyveTurleri = cell(meyveSayisi, 1);
    for i = 1:meyveSayisi
    ...
    [dosyaAdi, dosyaYol] = uigetfile('meyve_resimler/*.jpg', 'Meyve Seç');
    if dosyaAdi ~= 0
        resim = imread(fullfile(dosyaYol, dosyaAdi));
        [~, secilenResimAdi, ~] = fileparts(dosyaAdi);
        disp(['Seçilen Resim Adý: ' secilenResimAdi]);
...
elseif kirmizi_piksel_sayisi == 6 && yesil_piksel_sayisi == 910 && mavi_piksel_sayisi == 7
    meyveTur = 'Karpuz';    
elseif kirmizi_piksel_sayisi == 9 && yesil_piksel_sayisi == 944 && mavi_piksel_sayisi == 11
    meyveTur = 'Karpuz';    
...
elseif kirmizi_piksel_sayisi == 0 && yesil_piksel_sayisi == 0 && mavi_piksel_sayisi == 3422
    meyveTur = 'Yaban Mersini';
elseif kirmizi_piksel_sayisi == 0 && yesil_piksel_sayisi == 0 && mavi_piksel_sayisi == 3389
    meyveTur = 'Yaban Mersini';    
elseif kirmizi_piksel_sayisi == 0 && yesil_piksel_sayisi == 0 && mavi_piksel_sayisi == 3416
    meyveTur = 'Yaban Mersini';    
...
elseif kirmizi_piksel_sayisi == 531 && yesil_piksel_sayisi == 0 && mavi_piksel_sayisi == 0
    meyveTur = 'Zencefil';
elseif kirmizi_piksel_sayisi == 343 && yesil_piksel_sayisi == 0 && mavi_piksel_sayisi == 0
    meyveTur = 'Zencefil';
...
subplot(2, 1, 2);
tahminMetni = sprintf('Tahmin edilen meyve: %s', meyveTur);
text(0.5, 0.5, tahminMetni, 'FontSize', 20, 'HorizontalAlignment', 'center');
axis off;
    end
end
